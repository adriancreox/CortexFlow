...
class Scheduler:
    ...
    def __init__(
        ...
        self._vaults: dict[str, "MemoryVault"] = {} # Global registry reference
            ...
            try:
                ...
                async with lock.acquire(key=f"agent:{agent_id}", ttl=30):
                    ...
                    try:
                        ...
                        # 6. SHIELD GOVERNANCE: Validate tool calls before execution
                        ...
                        # 7. TOOL RUNNER: Ejecuta solo herramientas autorizadas
                        if tool_calls:
                            for tool_call in tool_calls:
                                try:
                                    # Llamada física a la función
                                    # Fix: Use 'call' instead of 'execute' and pass arguments as dict
                                    ...
                                    # CREACIÓN DEL EVENTO DE RESULTADO
                                    ...
                                    # Notificar error al agente para que razone sobre el fallo
